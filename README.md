...
- **Evidências da organização:** *[preencher — endereço completo, telefone/e-mail de contato, responsável pela organização, e link (Google Maps/Google Meu Negócio, site ou rede social) e/ou fotos da visita que comprovem o acesso real à clínica]*
...
> Os valores de exemplo abaixo são **fictícios**, usados apenas para ilustrar o tipo de dado — nenhum paciente ou dentista real é citado.

### Entidade: Paciente

| Atributo | Descrição | Regra de negócio associada |
|----------|-----------|------------------------------|
| id_paciente | Identificador único do paciente, gerado pelo sistema (ex.: PAC001) | Obrigatório; resolve a ausência de identificador único hoje observada |
| nome | Nome completo do paciente (ex.: "Maria da Silva") | Obrigatório |
| telefone_principal | Telefone de contato principal (ex.: "(11) 91234-5678") | Obrigatório |
| telefone_secundario | Telefone de contato adicional, se houver | Opcional; paciente pode ter mais de um contato |
| data_nascimento | Data de nascimento do paciente | Opcional na coleta atual, recomendado para o sistema |
| informacoes_saude | Informações de saúde relevantes coletadas na anamnese (ex.: "alergia a anestésico X") | Acesso restrito à dentista |
| data_cadastro | Data em que o paciente foi cadastrado na clínica | Preenchido automaticamente pelo sistema |

### Entidade: Dentista

| Atributo | Descrição | Regra de negócio associada |
|----------|-----------|------------------------------|
| id_dentista | Identificador único do dentista | Obrigatório |
| nome | Nome do profissional (ex.: "Tamires Souza") | Obrigatório |
| especialidade | Especialidade(s) do dentista (ex.: "Implantodontia") | Um dentista pode ter mais de uma especialidade (atributo multivalorado) |
...
| horario | Horário marcado | Um mesmo horário não pode ter dois pacientes com o mesmo dentista |
| canal_agendamento | Canal usado para marcar (ex.: "WhatsApp") | Hoje, sempre WhatsApp ou presencial mediante agendamento prévio |
| status | Situação do agendamento (agendado, confirmado, cancelado, remarcado, não compareceu) | Atualizado conforme o andamento |

### Entidade: Atendimento

| Atributo | Descrição | Regra de negócio associada |
|----------|-----------|------------------------------|
| id_atendimento | Identificador único do atendimento | Obrigatório |
| data_hora | Data e hora em que o atendimento efetivamente ocorreu | Obrigatório |
| observacoes | Observações registradas durante o atendimento | Fica registrado no histórico do paciente |
...
| nome | Nome do procedimento (ex.: "Limpeza", "Endodontia", "Implante") | Lista fixa de serviços oferecidos pela clínica |
| preco_base | Preço de referência do procedimento (ex.: "R$ 250,00") | Pode variar por paciente/situação (ver entidade associativa) |
| numero_sessoes_estimado | Quantidade estimada de sessões necessárias (ex.: 3) | Alguns procedimentos exigem várias sessões |
| procedimento_pre_requisito | Procedimento que precisa ser realizado antes (auto-relacionamento) | Alguns procedimentos exigem outro procedimento prévio |

### Entidade: Tratamento

| Atributo | Descrição | Regra de negócio associada |
|----------|-----------|------------------------------|
| id_tratamento | Identificador único do tratamento | Obrigatório |
| data_inicio | Data de início do tratamento | Obrigatório |
| data_fim | Data de conclusão do tratamento | Preenchido quando o tratamento é concluído |
| status | Situação do tratamento (em andamento, concluído) | — |
| evolucao | Anotações de evolução do tratamento (ex.: "Sessão 2 de 3 — paciente evoluindo bem") | Corresponde à "ficha de evolução" usada hoje em papel |
...
| valor | Valor pago naquela parcela/transação (ex.: "R$ 150,00") | Um atendimento pode ter mais de um pagamento (parcelamento) |
| forma_pagamento | Forma utilizada (pix, crédito, débito, dinheiro) | — |
| data_pagamento | Data em que o pagamento foi realizado | Pode ser antes ou depois do atendimento |
| status | Situação do pagamento (pago, pendente) | Paciente é notificado em caso de pendência |
